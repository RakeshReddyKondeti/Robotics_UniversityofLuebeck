...
class SARSA_lambda_Learner(object):
    ...
    def learn(self, obs, action, reward, done, next_obs, next_action):
        ...
        td_delta = reward + self.Q[discretized_next_sa_max] * (not done) - self.Q[discretized_sa]
        ...
        if(next_action_max == next_action):
            self.trace = self.gamma * self.trace * self.lam
        else:
...
if __name__ == "__main__":
    ...
    #np.save('watkins_Chris_Q' + str(agent.lam) + '_' + str(MAX_NUM_EPISODES) +'.npy', Q)
    np.save('watkins_3', episodic_returns)
