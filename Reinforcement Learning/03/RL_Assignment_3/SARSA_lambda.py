...
class SARSA_lambda_Learner(object):
    def __init__(self, env, traceUpdate):
        ...
        self.lam = 0.5 # lambda, to be modified in other tasks
    ...
    def learn(self, obs, action, reward, done, next_obs, next_action):
        ...
        self.Q += td_delta * self.alpha * self.trace
...
if __name__ == "__main__":
    ...
    np.save('SARSA_' + str(agent.lam) + '_' + str(MAX_NUM_EPISODES) +'_Q.npy', Q)
    np.save('SARSA_' + str(agent.lam) + '_' + str(MAX_NUM_EPISODES) +'_returns.npy', episodic_returns)
    #np.save('SARSA_returns_1', episodic_returns)
