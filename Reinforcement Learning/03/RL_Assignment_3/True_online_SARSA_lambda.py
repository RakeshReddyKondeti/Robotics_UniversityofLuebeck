...
class SARSA_lambda_Learner(object):
    def __init__(self, env, traceUpdate):
        ...
        self.Qp = self.Q.copy() # previous Q values
        self.Qn = self.Q.copy() # next Q values
        self.visit_counts = np.zeros((self.obs_bins[0]+1, self.obs_bins[1]+1, self.action_shape))
        self.Isst = np.zeros((self.obs_bins[0]+1, self.obs_bins[1]+1, self.action_shape)) # Equation ... from protocoll
    ...
    def learn(self, obs, action, reward, done, next_obs, next_action):
        ...
        td_delta = reward + self.Q[discretized_next_sa] * (not done) - self.Q[discretized_sa]
        ...
        # Update IsS_t
        self.Isst = self.Isst*0
        self.Isst[discretized_sa] = 1

        # To Do: update the q table
        self.Qn = self.Q + self.alpha*(td_delta+self.Q[discretized_sa]-self.Qp[discretized_sa])*self.trace - self.alpha*self.Isst*(self.Q[discretized_sa]-self.Qp[discretized_sa])
        self.Qp = self.Q
        self.Q = self.Qn
...
if __name__ == "__main__":
    ...
    #np.save('true_SARSA_1' + str(agent.lam) + '_' + str(MAX_NUM_EPISODES) +'.npy', Q)
    np.save('true_SARSA_3', episodic_returns)
