...
class SARSA_Learner(object):
    ...
    def get_action(self, state):
        ...
        if sample > 1-self.epsilon:
            ...
            return np.argmax([self.Q[discretized_state[0], discretized_state[1], 0], self.Q[discretized_state[0], discretized_state[1], 1], self.Q[discretized_state[0], discretized_state[1], 2]])
    ...
    def update_Q_table(self, obs, action, reward, done, next_obs):
        ...
        '''
        discredobs = self.discretize(obs)
        discrednextobs = self.discretize(next_obs)

        self.Q[discredobs[0], discredobs[1], action] += self.alpha * ( reward + self.gamma*(not done)*self.Q[discrednextobs[0], discrednextobs[1], action] - self.Q[discredobs[0], discredobs[1], action] )
...
def train(agent, env, MAX_NUM_EPISODES):
    ...
    for episode in range(MAX_NUM_EPISODES):
        ...
        while not done:
            ...
            discredspace = agent.discretize(obs)
            agent.visit_counts[discredspace[0], discredspace[1], action]
    ...
    np.save("returns3_SARSA", episodic_returns)
